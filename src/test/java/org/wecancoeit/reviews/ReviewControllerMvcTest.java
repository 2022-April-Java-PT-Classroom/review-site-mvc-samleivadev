...
public class ReviewControllerMvcTest {
    ...
    @Test
    public void shouldWorkForOneReviewInTheReviewTemplate() throws Exception {
        Long reviewOneId = 1L;
    ...
    @Test
    public void shouldFindOneCourseInModel() throws Exception {
        Long reviewOneId = 1L;
        when(reviewRepo.findOne(reviewOneId)).thenReturn(reviewOne);
        mockMvc.perform(get("/review?id=1"))
                .andExpect(status().isOk()).andExpect(model().attribute("reviewModel", reviewOne));
    }
